...
equipo3 = '9b56e023d84c4c0e9af2d0ee95549392' # IBERO-METEORO
...
boton5 = types.KeyboardButton('🚦 IQA')
...
boton10 = types.KeyboardButton('🌅 Radiación')
...
boton19 = types.KeyboardButton('✴ Hoy')
boton20 = types.KeyboardButton('📃 Semanal')

# Definición de botones del menú de "Radiación"
boton21 = types.KeyboardButton('☀ Hoy')
boton22 = types.KeyboardButton('😎 Semanal')
...
def send_main_menu(message):
    fscanf('Historial.txt', message, ab)
...
# Definición de submenu "Radiación"
@bot.message_handler(func=lambda message: message.text == '🌅 Radiación')
def send_submenu_t(message):
    bot.send_message(message.chat.id, '¿Qué rango te gustaría analizar?', reply_markup=submenu_viento_inter)
...
# Actividad ICA
@bot.message_handler(func=lambda message: message.text == '🚦 IQA')
def boton5_controlador(message):
    chat_id = message.chat.id
    mensaje_pin_id = enviar_pin(chat_id)
    dato = categoria_aire_f(equipo2)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
    bot.reply_to(message, dato)
    quitar_pin(chat_id, mensaje_pin_id)
...
def boton12_controlador(message):
    ...
    max_value = generar_grafica(equipo, temperatura, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton13_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, temperatura, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton14_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, humedad, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton15_controlador7(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, humedad, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton16_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, pm25, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton17_controlador7(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, pm25, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton18_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, pm10, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton19_controlador7(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, pm10, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton14_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, ozono, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton15_controlador7(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, ozono, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton16_controlador(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, co, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
def boton17_controlador7(message):
    ...
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo, co, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
# Actividad Radiación Hoy
@bot.message_handler(func=lambda message: message.text == '☀ Hoy')
def boton16_controlador(message):
    chat_id = message.chat.id
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo3, radia, 1)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))

    # Se envía el valor máximo
    bot.send_message(message.chat.id, f"Valor más alto: {max_value} W/m2")
    quitar_pin(chat_id, mensaje_pin_id)

# Actividad Radiación Semanal
@bot.message_handler(func=lambda message: message.text == '😎 Semanal')
def boton17_controlador7(message):
    chat_id = message.chat.id
    mensaje_pin_id = enviar_pin(chat_id)
    # Función para graficar
    max_value = generar_grafica(equipo3, radia, 7)
    bot.send_photo(message.chat.id, open('graph.png', 'rb'))
...
@bot.message_handler(func=lambda message: message.text == '🎧 Reproducir Audio')
def play_music(message):
    #play_obj = wave_obj.play()
    bot.send_message(message.chat.id, "🎵 Reproduciendo audio...")
    bot.send_message(message.chat.id, "Somos un grupo enfocado a hacer conocer la actividad meteorologica, con ayuda de graficas y diferentes herramientas que te brindaran una mejor experiencia.")

# Actividad detener musica
@bot.message_handler(func=lambda message: message.text == '⏹ Detener Audio')
def stop_music(message):
    #mixer.music.stop()  # Stop playing the music
    bot.send_message(message.chat.id, "⏹ Audio detenido")
